            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h1 class="text-3xl font-bold bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">Product Management</h1>
                            <p class="mt-1 text-slate-600">Manage your furniture catalog</p>
                        </div>
                    </div>
                    <button @click="openAddModal" class="px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-lg font-semibold hover:from-emerald-700 hover:to-teal-700 transition-all shadow-lg hover:shadow-xl flex items-center gap-2">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="bg-white rounded-xl shadow-lg border border-slate-200" data-aos="fade-up" data-aos-delay="100">
                <div class="p-6 space-y-6">
                    <div v-for="product in products" :key="product.id" class="flex items-center gap-4 p-4 border-2 border-slate-200 rounded-xl hover:border-emerald-300 hover:shadow-md transition-all">
                        ...
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-1">
                                ...
                                <span v-if="product.is_featured" class="px-2 py-1 text-xs font-bold rounded-full bg-amber-100 text-amber-700">⭐ Featured</span>
                                <span v-if="!product.is_active" class="px-2 py-1 text-xs font-bold rounded-full bg-red-100 text-red-700">Inactive</span>
                            </div>
                            <p class="text-sm text-slate-600 mb-2">{{ product.description || 'No description' }}</p>
                            <div class="flex items-center gap-4 text-sm">
                                <span class="font-semibold text-emerald-600">₱{{ product.price }}</span>
                                <span class="text-slate-600">Stock: {{ product.stock }}</span>
                                <span class="px-2 py-1 rounded-md text-xs font-semibold bg-slate-100 text-slate-700">{{ product.category }}</span>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            <button @click="previewProduct(product)" class="px-3 py-2 bg-purple-100 text-purple-700 rounded-lg font-semibold hover:bg-purple-200 transition-colors text-sm flex items-center gap-1">
                            ...
                            <button @click="openEditModal(product)" class="px-3 py-2 bg-blue-100 text-blue-700 rounded-lg font-semibold hover:bg-blue-200 transition-colors text-sm">Edit</button>
                            <button @click="confirmDelete(product)" class="px-3 py-2 bg-red-100 text-red-700 rounded-lg font-semibold hover:bg-red-200 transition-colors text-sm">Delete</button>
                        </div>
                    </div>
