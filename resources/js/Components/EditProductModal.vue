                            ...
                            <form @submit.prevent="submitEditProduct" class="p-4 sm:p-6 space-y-3 sm:space-y-4">
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                                    <div class="sm:col-span-2">
                                        ...
                                        <input v-model="form.name" type="text" required class="w-full px-3 py-2 sm:px-4 sm:py-3 text-sm sm:text-base border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
                                    </div>
                                    <div class="sm:col-span-2">
                                        <label class="block text-xs sm:text-sm font-semibold text-slate-700 mb-1.5 sm:mb-2">Description</label>
                                        <textarea v-model="form.description" rows="2" class="w-full px-3 py-2 sm:px-4 sm:py-3 text-sm sm:text-base border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"></textarea>
                                    </div>
                                    <div>
                                        <label class="block text-xs sm:text-sm font-semibold text-slate-700 mb-1.5 sm:mb-2">Price (₱)</label>
                                        <input v-model="form.price" type="number" step="0.01" required min="0" class="w-full px-3 py-2 sm:px-4 sm:py-3 text-sm sm:text-base border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
                                    </div>
                                    <div>
                                        <label class="block text-xs sm:text-sm font-semibold text-slate-700 mb-1.5 sm:mb-2">Stock</label>
                                        <input v-model="form.stock" type="number" required min="0" class="w-full px-3 py-2 sm:px-4 sm:py-3 text-sm sm:text-base border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
                                    </div>
                                    <div>
                                        <label class="block text-xs sm:text-sm font-semibold text-slate-700 mb-1.5 sm:mb-2">Category</label>
                                        <select v-model="form.category" required class="w-full px-3 py-2 sm:px-4 sm:py-3 text-sm sm:text-base border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                                    ...
                                    <div class="sm:col-span-2 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
                                        <label class="flex items-center gap-2">
                                            <input v-model="form.is_featured" type="checkbox" class="w-4 h-4 sm:w-5 sm:h-5 text-blue-600 border-slate-300 rounded focus:ring-blue-500">
                                            <span class="text-xs sm:text-sm font-medium text-slate-700">Featured (Max 8)</span>
                                        </label>
                                        <label class="flex items-center gap-2">
                                            <input v-model="form.is_active" type="checkbox" class="w-4 h-4 sm:w-5 sm:h-5 text-blue-600 border-slate-300 rounded focus:ring-blue-500">
                                ...
                                <div class="flex flex-col-reverse sm:flex-row items-center justify-end gap-2 sm:gap-3 pt-3 sm:pt-4 border-t border-slate-200">
                                    ...
                                    <button type="submit" :disabled="form.processing" class="w-full sm:w-auto px-6 py-2 text-sm sm:text-base bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg font-semibold hover:from-blue-700 hover:to-indigo-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
...
const emit = defineEmits(['close', 'productUpdated', 'toast']);
...
function submitEditProduct() {
    form.post(`/admin/products/${form.id}`, {
        ...
        onSuccess: () => {
            ...
            emit('toast', { show: true, message: 'Product updated successfully', type: 'success' });
        },
        onError: (errors) => {
            const firstError = Object.values(errors)[0];
            emit('toast', { show: true, message: firstError || 'Failed to update product', type: 'error' });
        },
    });
}
</script>
